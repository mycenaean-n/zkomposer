...
const aggregate = new Func<
  [calls: Call[]],
  {},
  { blockNumber: bigint; returnData: string[] }
>(abi, abi.getFunction('aggregate')!.selector);

const try_aggregate = new Func<
  [requireSuccess: boolean, calls: Array<[target: string, callData: string]>],
  {},
...
export class Multicall extends ContractBase {
  ...
  async tryAggregate(...args: any[]): Promise<any[]> {
    ...
    let results = new Array(size);
    for (let [from, to] of splitIntoPages(size, page)) {
      let response = await this.eth_call(try_aggregate, [
        false,
        calls.slice(from, to),
      ]);
      for (let i = from; i < to; i++) {
